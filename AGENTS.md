...
delivered filenames carry the barcode bcl-convert actually demuxed with: read `renaming_map_{config_id}_effective.csv`, never the workbook map, for anything whose name embeds a barcode. `pick_orientation` and `generate_effective_renaming_map` are both checkpoints for exactly that reason — call `await_orientation_decision(config_id)` before expanding such targets, and never swallow the exception it raises (that exception is snakemake deferring the expansion). the gate waits on the effective map, not the decision json: between those two the decision exists and the map does not, and expanding there names every target off the workbook map. a rule that reads the map from `params:` or `resources:` must also list `results/{config_id}/renaming_map_{config_id}_effective.csv` under `input:` — snakemake rejects a params function reaching an undeclared checkpoint, and each spawned slurm job rebuilds the DAG
